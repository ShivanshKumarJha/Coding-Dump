...
-- 2. Create new column containing average rating_count of restaurants through the dataset
select *, round(avg(rating_count) over(),0) as 'avg_rating_count' from restaurants;

-- 3. Create new column containing average cost of restaurants through the dataset
select *, round(avg(cost) over(),2) as 'avg_cost' from restaurants;

-- 4. Create column containing average, min, max of cost, rating, rating_count of restaurants through the dataset
...
select *, round(avg(cost) over(partition by name),0) as 'avg_cost' from restaurants;

-- 7. Create both column together
select *, 
	round(avg(cost) over(partition by city),2) as 'avg_cost_city',
    round(avg(cost) over(partition by name),0) as 'avg_cost_cuisine'
from restaurants;
...
select * from 
	(select *,avg(cost) over(partition by cuisine) as 'avg_cost' from restaurants) t 
